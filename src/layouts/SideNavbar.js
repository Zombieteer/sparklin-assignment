...
import { navLinks, navQuickLinks } from "./sidebarItems";
...
const SideNavbar = () => {
  ...
  return (
    <Grid item className='sideBar'>
      {navLinks.map((items) => (
        <Grid key={items.id}>
          <Button className={`sidebarItem ${activeLink === items.id ? `activeLink` : ''}`} onClick={(e) => manipulateOpenLinks(e, items)}>
          ...
          {openLinks.includes(items.id) && items.children.length > 0 ? (
            items.children.map((childItem) => (
              <Grid key={childItem.id}>
                <Button className={`sidebarItem subSidebarItem ${activeLink === childItem.id ? `activeLink` : ''}`} onClick={(e) => manipulateOpenLinks(e, childItem)}>
      ...
      {navQuickLinks.map((items) => (
        <Grid key={items.id}>
          <Button className={`sidebarItem ${activeLink === items.id ? `activeLink` : ''}`} 
          ...
          {openLinks.includes(items.id) && items.children.length > 0 ? (
            items.children.map((childItem) => (
              <Grid key={childItem.id}>
                <Button className={`sidebarItem subSidebarItem quicklinksItems ${activeLink === childItem.id ? `activeLink` : ''}`} 
